...
namespace Langben.DAL
{
    ...
    public partial class SysLogRepository : BaseRepository<SysLog>, IDisposable
    {
        ...
        public IQueryable<SysLog> DaoChuData(SysEntities db, string order, string sort, string search, params object[] listQuery)
        {
            ...
            DateTime? strCreateTimeStart_Time=null ;
            DateTime? strCreateTimeEnd_Time=null  ;
            ...
            if (queryDic != null && queryDic.Count > 0)
            {
                ...
                {
                    if (!string.IsNullOrWhiteSpace(item.Key) && !string.IsNullOrWhiteSpace(item.Value) && item.Key.Contains(Start_Time)) // 
                    {
                        strCreateTimeStart_Time =Convert.ToDateTime( item.Value);
                        continue;
                    }
                    if (!string.IsNullOrWhiteSpace(item.Key) && !string.IsNullOrWhiteSpace(item.Value) && item.Key.Contains(End_Time)) // 
                    {
                        strCreateTimeEnd_Time = Convert.ToDateTime(item.Value).AddDays(1);
                      continue;
                    }
                    if (flagWhere != 0)
                    {
            ...
            if (null!=strCreateTimeStart_Time)
            {
                datas = datas.Where(w => w.CreateTime >= strCreateTimeStart_Time);
            }
            if (null != strCreateTimeEnd_Time)
            {
                datas = datas.Where(w => w.CreateTime < strCreateTimeEnd_Time);
            }
            return datas;
